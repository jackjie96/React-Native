...
import {
	GET_RN_OPEN_REPO, RECEIVE_RN_OPEN_REPO, SEARCH_REPO, SEARCH_REPO_RESULT,
} from '../actions/actionTypes';

function* getRNOpenRepo(action) {
	try {
		let { data } = yield axios.get(`https://api.github.com/orgs/react-native-community/repos?page=${action.page}&per_page=${action.dispLimit}`);
...
function* searchRepo(action) {
	try {
		let { data } = yield axios.get(`https://api.github.com/search/repositories?q=${action.keyword}&page=${action.page}&per_page=${action.dispLimit}`)
